<div class="col-lg-12">
    <div class="card">
        <div class="card-body">
            <div class="right-aligned">
                <?php if(!empty($search)) {?>
                    <a href="<?= base_url('/home/loan/')?>"><button class="btn btn-info"><i class="fa fa-undo"></i> Back</button></a>
                <?php }?>
                <?php if(session()->get('level')== 2 || session()->get('level')== 3) { ?>
                <a href="<?= base_url('/home/add_loan/')?>"><button class="btn btn-success"><i class="fa fa-plus"></i> Add</button></a>
                <?php }else{} ?>
            </div>
            <style>
                ...
                <table class="table items-table table table-bordered table-striped verticle-middle table-responsive-sm">
                    ...
                    <tbody>
                        ...
                        foreach ($duar as $gas){
                          ...
                          <tr>
                            <td>
                                <input type="checkbox" class="checkbox__input" value="<?= $gas->id_pinjaman ?>" name="Loan[]" id="Loan<?= $gas->id_pinjaman ?>"/>
                            </td>
                            <td class="text-center text-capitalize text-dark"><?php echo $gas->nama_pinjaman?></td>
                            <td class="text-center text-capitalize text-dark"><?php echo $gas->nominal?></td>
    ...
    function displayDataOnPage(page) {
        ...
        for (let i = startIndex; i < endIndex && i < data.length; i++) {
            ...
            <td class="text-center text-capitalize text-dark">${gas.nominal}</td>
            <td class="text-center text-capitalize text-dark">${gas.status_acc}</td>
